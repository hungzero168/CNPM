...
$result_thanhvien=mysqli_query($conn,"SELECT * FROM thanhvien where user='$idkey'");
...
if(isset($_POST["act"])){	
	...
	$result_thanhvien2=mysqli_query($conn,$sql);
	...
	else
	{
		...
		{
			$pw=md5($pw);	
			$result_changepw = mysqli_query($conn,"UPDATE thanhvien SET pass='$pww' WHERE user='$user'");
...
<td colspan="2" >ĐỔI MẬT KHẨU</td>
 ...
 <tr height="30" bgcolor="#F9F9F9" onmouseover="style.background='#d4340c'" onmouseout="style.background='#F9F9F9'">
	...
	<tr height="30" bgcolor="#F9F9F9" onmouseover="style.background='#d4340c'" onmouseout="style.background='#F9F9F9'">
      <td style="padding-left:100px">Nhập lại mật khẩu mới:</td>
