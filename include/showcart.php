...
if(isset($_POST["act"]))
{
	if(isset($_POST["capnhatgiohang"]))
	{
		...
		if(isset($_SESSION["user"]))
		{
			...
			{			
				if($sl[$i]<=0)
					$del=mysqli_query($conn,"delete from giohang where id='$id_form[$i]' and tinhtrang='themgiohang'");
				else{		
				$tt[$i]=$tong[$i]*$sl[$i];
...
//		$tong=$_POST["tong"];
		...
		//	echo "$sql_kt<hr>";
			...
			{		
				...
				$kq_dathang=mysqli_query($conn,$sql_dathang);			
			...
			else
			{
				...
				//	echo "$sl_kt<hr>";
					...
					$kq_del=mysqli_query($conn,$sql_del);
